...
//   const fetchDashboard = async () => {
//     try {
//       const response = await api.get('/api/dashboard') // ✅ Use live backend
//       setDashboardData(response.data)
//       setLoading(false)
//     } catch (error) {
//       console.error('Error fetching dashboard:', error)
...
//             </h2>
//             <p className="text-gray-400 text-lg">
//               {dashboardData?.productivity_insight || 'Ready to crush your goals today?'}
//             </p>
//           </div>
//           <ProductivityRing score={dashboardData?.productivity_score || 0} />
//         </div>
//       </div>
...
//         {focusPrediction && (
//           <div className="glass-panel p-4 sm:p-6 border border-cyber-blue/20">
//             <p className="text-xs text-gray-400 mb-1 font-mono">AI FOCUS PREDICTION</p>
//             <h3 className="text-lg font-semibold text-white">
//               Best focus window: <span className="text-cyber-blue">{focusPrediction.window}</span>
//             </h3>
//             <p className="text-sm text-gray-400 mt-1 max-w-2xl">{focusPrediction.message}</p>
//           </div>
//         )}
...
//         {productivityPersona && (
//           <div className="glass-panel p-4 sm:p-6 border border-cyber-blue/20">
//             <p className="text-xs text-gray-400 mb-1 font-mono">AI PRODUCTIVITY PERSONA</p>
//             <h3 className="text-lg font-semibold text-white">{productivityPersona.persona}</h3>
//             <p className="text-sm text-gray-400 mt-1 max-w-2xl">{productivityPersona.description}</p>
//           </div>
//         )}
//       </div>
...
//           <div className="flex items-center justify-between mb-4">
//             <div className="text-4xl">✓</div>
//             <div className="text-3xl font-display font-bold text-success-green">{dashboardData?.tasks_completed_today || 0}</div>
//           </div>
//           <div className="text-sm text-gray-400">Tasks Completed</div>
//           <div className="mt-2 text-xs text-success-green font-mono">TODAY</div>
//         </div>
...
//           <div className="flex items-center justify-between mb-4">
//             <div className="text-4xl">🎯</div>
//             <div className="text-3xl font-display font-bold text-cyber-blue">{dashboardData?.focus_sessions_today || 0}</div>
//           </div>
//           <div className="text-sm text-gray-400">Focus Sessions</div>
//           <div className="mt-2 text-xs text-cyber-blue font-mono">TODAY</div>
//         </div>
...
//           <div className="flex items-center justify-between mb-4">
//             <div className="text-4xl">⏱️</div>
//             <div className="text-3xl font-display font-bold text-focus-purple">{dashboardData?.total_focus_minutes || 0}</div>
//           </div>
//           <div className="text-sm text-gray-400">Focus Minutes</div>
//           <div className="mt-2 text-xs text-focus-purple font-mono">TODAY</div>
//         </div>
...
//           <div className="flex items-center justify-between mb-4">
//             <div className="text-4xl">🧠</div>
//             <div className={`text-3xl font-display font-bold ${getFocusLevelClass(dashboardData?.current_focus_level)}`}>
//               {dashboardData?.current_focus_level || 'N/A'}
//             </div>
//           </div>
//           <div className="text-sm text-gray-400">Current Focus</div>
...
import { useState, useEffect } from 'react'
import { api } from '../api'  // ✅ Using api module
...
function getFocusPrediction(data) {
  ...
  if (total_focus_minutes >= 120) {
    return {
      window: 'Evening',
      message: 'Your productivity spikes later in the day. Schedule deep work after sunset.'
    }
  }
  ...
  if (focus_sessions_today >= 2) {
    return {
      window: 'Afternoon',
      message: 'Mid-day is your strongest focus zone. Protect it from distractions.'
    }
  }

  return {
    window: 'Morning',
    message: 'Your focus is highest early in the day. Start with your hardest task.'
  }
}
...
function getProductivityPersona(data) {
  ...
  if (total_focus_minutes >= 120 && current_focus_level === 'High') {
    return {
      persona: 'Deep Thinker',
      description: 'You excel in long, focused sessions. Perfect for coding, writing, and analysis.'
    }
  }
  ...
  if (tasks_completed_today >= 5 && total_focus_minutes < 90) {
    return {
      persona: 'Sprinter',
      description: 'You work best in short bursts. Quick wins and high-intensity tasks are your strength.'
    }
  }

  return {
    persona: 'Marathoner',
    description: 'You maintain moderate focus consistently. Ideal for steady progress throughout the day.'
  }
}
...
function Dashboard() {
  ...
  const dailyTip = focusTips[Math.floor(Math.random() * focusTips.length)]
  ...
  const fetchDashboard = async () => {
    try {
      const response = await api.get('/api/dashboard')  // ✅ FIXED
      setDashboardData(response.data)
      setLoading(false)
    } catch (error) {
      console.error('Error fetching dashboard:', error)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-cyber-blue/30 border-t-cyber-blue rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-gray-400">Loading your productivity data...</p>
        </div>
      </div>
    )
  }
